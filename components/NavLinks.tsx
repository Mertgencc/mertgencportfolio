  ...
  return (
    <div className="relative">
      ...
      <button 
        ...
        className="lg:hidden flex items-center gap-2 px-4 py-2 bg-black/[0.02] dark:bg-white/[0.02] border border-black/10 dark:border-white/10 text-black dark:text-white"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-[#0A0A0A] border border-black/10 dark:border-white/10 shadow-2xl flex flex-col lg:hidden z-[200]"
          >
            {links.map((link) => {
              ...
              return (
                <Component
                  ...
                  className="px-5 py-4 border-b border-black/5 dark:border-white/5 last:border-0 flex items-center gap-3"
                >
                  <div className="text-cyan-500">{renderIcon(link.label)}</div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-black dark:text-white">{link.label}</span>
                </Component>
              );
            })}
          </motion.div>
...
const renderIcon = (label: string) => {
  ...
  switch (label) {
    ...
    case "Deneyimler": return <svg {...props} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"></rect><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"></path></svg>;
    default: return null;
  }
};
